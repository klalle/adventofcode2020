...
class One {
    ...
    fun run2(list: List<Long> = costList, sum: Long=2020) {//part 2
        var i=0;
        while(i<costList.size){
            var j=i
            while(j<costList.size) {
                var k=j
                while(k<costList.size) {
                    if (list[i] + list[j]+ list[k] == sum) {
                        //println("i:$i, j:$j, k:$k, [${costList[i]}x${costList[j]}x${costList[k]}]=${costList[i] * costList[j]*costList[k]}")
                        return;
                    }
                    k++
                }
